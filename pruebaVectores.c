...
//  [   [ proceso, proceso, proceso ], [ proceso, proceso ] ]
    ...
    for (int i = 0; i < vector_total(lista_de_listas); i++) { // Recorrer lista gigante
        ...
        printf("Tamaño lista PEQUEÑA %d",vector_total(lista_seleccionada));

        for (int j = 0; j < vector_total(lista_seleccionada); j++) { // Recorrer lista pequeña

            struct proceso *proceso_seleccionada = (struct proceso*) vector_get(lista_seleccionada, j);
